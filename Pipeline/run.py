...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_data", type=str, default="data", help="the dataset folder"
    )
    parser.add_argument(
    ...
    # find all the folders in the folder with the prefix datadir

    folder_path = os.path.abspath(FOLDER)
    datadir_folders = [
        os.path.basename(f)
        for f in glob.glob(os.path.join(folder_path, "datadir*"))
        if os.path.isdir(f)
    ]

    print(datadir_folders)

    for folder in datadir_folders:
