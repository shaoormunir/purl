...
def get_cookie_features(G, df_graph, node):
  ...
  for item in cookie_set_edge_attr:
    try:
      if "N/A" not in item and not pd.isnull(item):
...
def get_content_additional(G, node):
  ...
  # get attributes of node from the graph
  try:
    value = json.loads(G.nodes[node]['attr'])['value']
    shannon_entropy = calculate_shannon_entropy(value)
  except Exception as e:
    print("Error in shannon entropy", e)
    print("Node", node)
    shannon_entropy = -1
  max_depth_decoration = -1
  if '||path||' in node:
    # www.adobe.com||www.adobe.com||path||3 -> 3
    try:
      max_depth_decoration = int(node.split("||")[-1])
    except Exception as e:
      print("Error in max_depth_decoration", e)
      print("Node", node)
      print(node.split("||")[-1])
      print(node.split("||"))
  
  content_features = [shannon_entropy, max_depth_decoration]
  content_feature_names = ['shannon_entropy', 'max_depth_decoration']
...
def get_additional_features(G, df_graph, node):
  ...
  all_features = []
  all_feature_names = []
  content_features, content_feature_names = get_content_additional(G, node)
  ...
  all_feature_names = content_feature_names
