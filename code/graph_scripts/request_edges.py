...
def get_redirect_edges(df_requests, df_redirects, df_responses):
    df_reqheaders = df_requests[
        ["visit_id", "request_id", "url", "headers", "top_level_url", "time_stamp"]
    ]
    df_red = df_redirects[
        [
            "visit_id",
    ...
    x1 = pd.merge(
        df_red,
        df_reqheaders,
        left_on=["visit_id", "old_request_id", "old_request_url"],
        right_on=["visit_id", "request_id", "url"],
    )
    x2 = pd.merge(
        x1,
        df_requests,
        left_on=["visit_id", "old_request_id", "new_request_url"],
        right_on=["visit_id", "request_id", "url"],
    )
    x2 = x2.rename(
        columns={
...
def get_cs_edges(df_requests, df_responses, call_stacks):
    ...
    call_stack_nodup = (
        call_stacks[["visit_id", "request_id", "call_stack"]].drop_duplicates().copy()
    )
    df_merge = pd.merge(
...
def build_request_components(df_requests, df_responses, df_redirects, call_stacks):
    ...
    try:
        ...
        df_request_nodes = (
            df_requests[["visit_id", "url", "type", "top_level_url", "attr"]]
            .drop_duplicates()
            .copy()
        )
