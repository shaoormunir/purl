...
def bot_mitigation(webdriver):
    ...
    n_atags_to_open = random.randint(15, 20)
    anchor_tags = webdriver.find_elements_by_tag_name('a')
    # remvoe duplciate anchor tags
    anchor_tags = list(set(anchor_tags))

    print('number of total anchor tags on the page: ', len(anchor_tags))
    ...
    if not os.path.exists(f'urls/{parent_domain}'):
        os.makedirs(f'urls/{parent_domain}')

    if os.path.exists(f'urls/{parent_domain}/urls.json'):
        print('The file already exists, loading tags...')
        new_anchor_tags =  []
        print('Loading tags')
        with open(f'urls/{parent_domain}/urls.json', 'r') as f:
            url_dict = json.load(f)
        keys = list(url_dict.keys())
        urls = url_dict[keys[0]]
        ...
        for anchor_tag in anchor_tags:
            ...
            if element_url in urls:
                print('found')
                new_anchor_tags.append(anchor_tag)
        if len(new_anchor_tags) > len(urls):
            # select a random subset of the anchor tags equal to the number of original anchor tags
            new_anchor_tags = random.sample(new_anchor_tags, len(urls))
        anchor_tags = new_anchor_tags
    else:
        print('Creating tags')
        new_anchor_tags = []
        # remove anchor tags that are not in the same domain
        for anchor_tag in anchor_tags:
            element_url = anchor_tag.get_attribute('href')
            if element_url is not None:
                if get_etldp1(element_url) == parent_domain:
                    new_anchor_tags.append(anchor_tag)

        if len(new_anchor_tags) > n_atags_to_open:
            # select a random subset of the anchor tags equal to the number of original anchor tags
            new_anchor_tags = random.sample(new_anchor_tags, n_atags_to_open)
        else:
            print('Not enough anchor tags')
        # save the anchor tags to the urls.json

        for anchor_tag in new_anchor_tags:
            element_url = anchor_tag.get_attribute('href')
            if element_url is not None:
